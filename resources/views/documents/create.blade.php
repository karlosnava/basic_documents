...
@section('content')
    <form action="{{ route('documents.store') }}" method="POST">
        ...
        <x-input label="Nombre del documento" name="doc_nombre" />

        <div class="mt-5">
            <textarea name="doc_contenido" required>{{ old('doc_contenido') }}</textarea>
            @error('doc_contenido')
            ...
            <div>
                <x-select label="Tipo de documento" name="doc_id_tipo">
                    @foreach ($tipo_docs as $tipo_doc)
                        <option value="{{ $tipo_doc->tip_id }}" @if($tipo_doc->tip_id == old('doc_id_tipo')) selected @endif>{{ $tipo_doc->tip_nombre }}</option>
                    @endforeach
                </x-select>
            </div>
            ...
            <div>
                <x-select label="Tipo de proceso" name="doc_id_proceso">
                    @foreach ($pro_procesos as $pro_proceso)
                        <option value="{{ $pro_proceso->pro_id }}" @if($pro_proceso->pro_id == old('doc_id_proceso')) selected @endif>{{ $pro_proceso->pro_nombre }}</option>
                    @endforeach
                </x-select>
            </div>
        </div>
